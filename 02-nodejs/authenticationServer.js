...
app.post("/signup/", (req, res) => {
  let exisitingUser = data.find((u) => u.email == req.body.email);
  if (exisitingUser === -1) {
...
app.get("/data/", (req, res) => {
  const userFound = data.find((a) => {
    if (a.email === req.headers.email && a.password === req.headers.password) {
      return a;
    }
    return 0;
  });
